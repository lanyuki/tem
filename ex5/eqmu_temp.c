...
void store_data_in_db(const char *dev_id, int dev_temp);
...
//		const char *json_str;		
	   ...
	   store_data_in_db(json_str, json_int);  //存入数据库	   
	   printf("\n===============\n\n"); 
...
/*数据库操作函数*/
void store_data_in_db(const char *dev_id, int temp)
{
	sqlite3 *db;
	...
	char *sql = "CREATE TABLE IF NOT EXISTS Temperature(Id varchar[20], Temp INTEGER);";
	...
	sql = sqlite3_mprintf("INSERT INTO Temperature(Id,Temp) values('%s',%d);",dev_id, temp);
